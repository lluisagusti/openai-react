...
export default function InputArea({ doAction, setInput, result, input }: any) {
  return (
    <div style={{ paddingLeft: '2em', paddingTop: '10em'}}>
      ...
      <p className="result-text">{result.length > 0 ? `${input[0].toUpperCase()}${input.slice(1)} ${result}` : ""}</p>
    </div>
  )
}